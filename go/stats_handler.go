...
func getUserStatisticsHandler(c echo.Context) error {
	...
	query = `
		SELECT u.id AS user_id, IFNULL(SUM(l2.tip), 0) AS total_tips 
		FROM users u
		INNER JOIN livestreams l ON l.user_id = u.id	
...
func getLivestreamStatisticsHandler(c echo.Context) error {
	...
	for _, livestream := range livestreams {
		var reactions int64
		if err := tx.GetContext(ctx, &reactions, "SELECT COUNT(*) FROM reactions r WHERE r.livestream_id = ?", livestream.ID); err != nil && !errors.Is(err, sql.ErrNoRows) {
			return echo.NewHTTPError(http.StatusInternalServerError, "failed to count reactions: "+err.Error())
		}

		var totalTips int64
		if err := tx.GetContext(ctx, &totalTips, "SELECT IFNULL(SUM(l2.tip), 0) FROM livecomments l2 WHERE l2.livestream_id = ?", livestream.ID); err != nil && !errors.Is(err, sql.ErrNoRows) {
			return echo.NewHTTPError(http.StatusInternalServerError, "failed to count tips: "+err.Error())
		}
